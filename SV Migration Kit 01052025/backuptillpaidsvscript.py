...
if __name__ == "__main__":
    ...
    if not api_reachable:
        print("\nWARNING: Could not confirm API connectivity to one or more endpoints or received 4xx/5xx errors during test. Please check your network, the API server status, the URLs, AUTH_TOKEN, and USER_INFO in the script.")
        print("Also, check the server logs for the 500 errors on Create/Update APIs.")
        print(f"Create API reachable: {create_api_reachable}")
        print(f"Update API reachable: {update_api_reachable}")
        print(f"Bill Fetch API reachable: {bill_fetch_api_reachable}")
        print(f"Payment API reachable: {payment_api_reachable}")

        proceed = input("Do you want to attempt the migration anyway despite potential API issues? (y/n): ")
        if proceed.lower() != 'y':
